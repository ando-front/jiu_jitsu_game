...
2. **Stage 2 — Unity 6 production build** in [src/prototype/unity/](src/prototype/unity/). Purpose: realise Visual Pillar Document requirements (photoreal characters, SSS, Gi cloth sim, URP post-processing, HUD-less color-grading state conveyance). Runs natively on Apple Silicon (macOS) and Windows PC without an RTX GPU requirement. **Status (2026-05-08): all Pure logic, EditMode tests (~270), PlayMode tests, and Platform layer green. Visual Pillar code-side complete — Transform-driven `BJJAvatarBinder` (primary), Animator-driven `BJJAnimatorBinder` (Mixamo Humanoid alt), URP `BJJVolumeController` (stamina warm shift / vignette), `BJJImpactFeedback` (pulseFlash / chromatic / camera shake), and UI Toolkit `BJJHud` (`.uxml` + `.uss`) all merged. Unity MCP integration (`com.coplaydev.unity-mcp` + repo `.mcp.json`) and `BJJ → Setup Scene` / `BJJ → Run Scenario` Editor automation in place. Remaining work is Editor-bound asset import (rigged characters, Animator Controllers, URP Volume profiles) which cannot progress from a headless cloud session. See [docs/design/stage2_port_plan_v1.md](docs/design/stage2_port_plan_v1.md), [docs/design/stage2_port_progress.md](docs/design/stage2_port_progress.md), and [src/prototype/unity/README.md](src/prototype/unity/README.md).**
...
- [docs/design/stage2_port_plan_v1.md](docs/design/stage2_port_plan_v1.md) — file-by-file TS → Unity C# port plan
...
- [src/prototype/unity/](src/prototype/unity/) — Stage 2 Unity 6 project (open in Unity Hub → Unity 6000.0 LTS). See [stage2_port_plan_v1.md](docs/design/stage2_port_plan_v1.md) before adding files.
